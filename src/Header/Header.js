...
const toggleoffcanva = () => {
  setOpen(!open);
  getcakeCartView();
};
  const getcakeCartView = async () => {
    try {
      ...
      const response = await axios.get(
       `${fetchgetcakeCartView}/${userid}`,
        ...
        console.log("cartview:", response.data.data);
        setCartadded(response.data.data)  
    } catch (error) {
      ...
      <div className="right-side-cart">
        ...
        <div>
          ...
          <Offcanvas style={{ backgroundColor: '#f9f6f0' }} isOpen={open} toggle={toggleoffcanva} direction="end">
            <OffcanvasHeader toggle={toggleoffcanva}><h4>Shopping Cart({cartCount})</h4></OffcanvasHeader>
            <OffcanvasBody>
                  ...
                  <CardHeader style={{ backgroundColor: 'rgb(211, 211, 211)' }}>
                    <Row>
                      ...
                      <Col md='6'>
                        ...
                        <span style={{ fontFamily: 'bold' }}>{cartCount} </span>
              ...
              <div className='place-order-btn'>
                ...
                <Link to='/buy'>  <button type='button' style={{ backgroundColor: '#fd7cc2', width: '300px', color: '#fff', position: "fixed", bottom: '1%', height: '50px', border: 'none', marginLeft: '30px' }}>Cart Order</button>
                </Link>
              </div>
            </OffcanvasBody>
          </Offcanvas>
